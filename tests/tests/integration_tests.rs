...
async fn new_verifier_succeeds() {
    ...
    let verifier_address: Address = {
        ...
        println!("Deploying Verifier Contract...");

        zk_wallet.deploy(&deploy_request).await.unwrap()
    };

    println!("Verifier Deployed at: {verifier_address:?}");
    ...
    println!("Calling Verify method with a valid proof...");
    let era_response = test_utils::era_call(
        verifier_address,
        None,
        Some(Bytes::from(hex::decode(calldata).unwrap())),
    ...
    if era_output == Bytes::from(RESPONSE_VALID) {
        println!("Verifier Returned: {}, Proof Verified!", era_output);
    } else {
        println!("Verifier Returned: {}, Proof did not verify", era_output);
    }
